// Test ComputeKeysK4 function directly
#include <stdio.h>
#include <cuda.h>
...
// Simplified ComputeKeysK4 - just do ONE group step
__device__ void ComputeKeysK4_Single(uint32_t mode, uint64_t* startx, uint64_t* starty, uint32_t maxFound, uint32_t* out) {
    ...
    // Just do j=0 iteration - ONE group
    uint32_t j = 0;
    uint32_t i;
    
    // Prepare denominators for batch inversion
    for (i = 0; i < HSIZE; i++)
        ModSub256(dx[i], Gx[i], sx);
    ModSub256(dx[i], Gx[i], sx);
    ModSub256(dx[i+1], _2Gnx, sx);

    // Batch inverse - THIS IS THE SLOW PART
    _ModInvGrouped(dx);

    // Check the starting point (offset = GRP_SIZE/2 = 512)
    CheckHashCompSymK4(px, py, tid, j*GRP_SIZE + GRP_SIZE/2, maxFound, out, false);

    // We won't do the full loop - just verify the first check works
}

__global__ void test_compute_kernel(uint64_t* keys, uint32_t maxFound, uint32_t* found) {
    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * NB_THREAD_PER_GROUP;
    ComputeKeysK4_Single(0, keys + xPtr, keys + yPtr, maxFound, found);
}
...
int main() {
    printf("=== ComputeKeysK4 Single Step Test ===\n\n");
    
    cudaDeviceSetLimit(cudaLimitStackSize, 64 * 1024);  // Increase stack size
    ...
    // Allocate keys for 1 block (64 threads)
    ...
    test_compute_kernel<<<1, NB_THREAD_PER_GROUP>>>(d_keys, MAX_FOUND, d_found);
