...
def validate_wif_key(wif_key):
    """Validate a Bitcoin WIF private key"""
    try:
        # Decode from base58
        decoded = base58.b58decode(wif_key)

        # WIF format: [version byte] [32 bytes private key] [optional compression flag] [4 bytes checksum]
        # Uncompressed: 37 bytes (1 + 32 + 4)
        # Compressed: 38 bytes (1 + 32 + 1 + 4)
        ...
        if decoded[0] != 0x80:
            return False, f"Invalid version byte: {hex(decoded[0])}"

        # Extract checksum (last 4 bytes)
        checksum = decoded[-4:]

        # Extract payload (everything except checksum)
        payload = decoded[:-4]
        ...
        if len(decoded) == 38:
            if decoded[33] != 0x01:
                return False, f"Invalid compression flag: {hex(decoded[33])}"
            key_type = "compressed"
        else:
...
keys = """5KC7FNcyy5P4o7tvyTr8SDNNsoQh6DyUdovubWamo7Ah6q71sNN
...
print("Validating Bitcoin WIF private keys...\n")
...
for key in keys:
    ...
    is_valid, result = validate_wif_key(key)

    if is_valid:
        valid_keys.append(key)
        print(f"✓ {key} - VALID ({result})")
    else:
        invalid_keys.append((key, result))
        print(f"✗ {key} - INVALID: {result}")
...
if valid_keys:
    ...
    # Save to file
    with open('/root/repo/valid_keys.txt', 'w') as f:
        for key in valid_keys:
            f.write(key + '\n')
    print(f"\nValid keys saved to: /root/repo/valid_keys.txt")
