...
__global__ void comp_keys_uncomp(prefix_t* prefix, uint32_t* lookup32, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysUnComp(keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);
}

__global__ void comp_keys_both(prefix_t* prefix, uint32_t* lookup32, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysBoth(keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

//

__global__ void comp_keys_comp_P2SH(prefix_t* prefix, uint32_t* lookup32, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysCompP2SH(keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

__global__ void comp_keys_uncomp_P2SH(prefix_t* prefix, uint32_t* lookup32, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysUnCompP2SH(keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

__global__ void comp_keys_both_P2SH(prefix_t* prefix, uint32_t* lookup32, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysBothP2SH(keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

///

__global__ void comp_keys_comp_Pattern(prefix_t* pattern, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysCompPattern(keys + xPtr, keys + yPtr, NULL, (uint32_t*)pattern, maxFound, found);

}

__global__ void comp_keys_uncomp_Pattern(prefix_t* pattern, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysUnCompPattern(keys + xPtr, keys + yPtr, NULL, (uint32_t*)pattern, maxFound, found);

}

__global__ void comp_keys_both_Pattern(prefix_t* pattern, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysBothPattern(keys + xPtr, keys + yPtr, NULL, (uint32_t*)pattern, maxFound, found);

}

//

__global__ void comp_keys_comp_P2SH_Pattern(prefix_t* pattern, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysCompP2SHPattern(keys + xPtr, keys + yPtr, NULL, (uint32_t*)pattern, maxFound, found);

}

__global__ void comp_keys_uncomp_P2SH_Pattern(prefix_t* pattern, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysUnCompP2SHPattern(keys + xPtr, keys + yPtr, NULL, (uint32_t*)pattern, maxFound, found);

}

__global__ void comp_keys_both_P2SH_Pattern(prefix_t* pattern, uint64_t* keys, uint32_t maxFound, uint32_t* found) {

    int xPtr = (blockIdx.x * blockDim.x) * 8;
    int yPtr = xPtr + 4 * blockDim.x;
    ComputeKeysBothP2SHPattern(keys + xPtr, keys + yPtr, NULL, (uint32_t*)pattern, maxFound, found);
  ...
  if (searchType == P2SH) {
    ...
    if (hasPattern) {
        ...
        case SEARCH_COMPRESSED:
            comp_keys_comp_P2SH_Pattern << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
                (inputPrefix, inputKey, maxFound, outputPrefix);
            break;

        case SEARCH_UNCOMPRESSED:
            comp_keys_uncomp_P2SH_Pattern << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
                (inputPrefix, inputKey, maxFound, outputPrefix);
            break;
        case SEARCH_BOTH:
            comp_keys_both_P2SH_Pattern << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
                (inputPrefix, inputKey, maxFound, outputPrefix);
            break;
        }
        ...
        case SEARCH_COMPRESSED:
            comp_keys_comp_P2SH << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
                (inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
            break;

        case SEARCH_UNCOMPRESSED:
            comp_keys_uncomp_P2SH << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
                (inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
            break;
        case SEARCH_BOTH:
            comp_keys_both_P2SH << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
                (inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
            break;
        }
    }
    ...
    if (hasPattern) {
      ...
      case SEARCH_COMPRESSED:
          comp_keys_comp_Pattern << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
              (inputPrefix, inputKey, maxFound, outputPrefix);
          break;

      case SEARCH_UNCOMPRESSED:
          comp_keys_uncomp_Pattern << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
              (inputPrefix, inputKey, maxFound, outputPrefix);
          break;
      case SEARCH_BOTH:
          comp_keys_both_Pattern << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
              (inputPrefix, inputKey, maxFound, outputPrefix);
          break;
      }
      ...
      case SEARCH_UNCOMPRESSED:
          comp_keys_uncomp << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
              (inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
          break;
      case SEARCH_BOTH:
          comp_keys_both << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
              (inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
          break;
      }
    }
