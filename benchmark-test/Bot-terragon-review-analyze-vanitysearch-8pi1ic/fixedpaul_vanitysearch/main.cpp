...
#define RELEASE "2.0 by FixedPaul"
...
int main(int argc, char* argv[]) {
  ...
  bool sse = true;
  uint32_t maxFound = 65536*128;
  //uint64_t rekey = 1099511627776;
  uint64_t rekey = (uint64_t)1 << 62;
  Point startPuKey;
  startPuKey.Clear();
  ...
  printf("GPU code not compiled, use -DWITHGPU when compiling.\n");
      ...
      a++;
        ...
        a++;
        int exponent = (int)getInt("rekey", argv[a]);
        if (exponent < 36) {
            exponent = 36;
        }
        if (exponent > 62) {
        ...
        rekey = (uint64_t)1 << exponent; // 2 elevato all'esponente
        a++;
