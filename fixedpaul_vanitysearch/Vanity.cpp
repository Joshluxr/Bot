...
void VanitySearch::getGPUStartingKeys(int thId, int groupSize, int nbThread, Int* keys, Point* p) {
    ...
    for (int i = 0; i < nbThread; i++) {
        keys[i].Set(&Start);
        Int off((uint64_t)i);
        off.Mult((uint64_t)(2*rekey / nbThread));
        keys[i].Add(&off);
    }

	p[grp_startkeys / 2] = secp->ComputePublicKey(&Start);
...
void VanitySearch::FindKeyGPU(TH_PARAM *ph) {
  ...
  while (ok && !endOfSearch) {
    ...
    if (ok) {
      ...
      counters[thId] += HashPerKeys * STEP_SIZE * nbThread; // Point +  endo1 + endo2 + symetrics
    }
...
uint64_t VanitySearch::getGPUCountRate() {
...
void VanitySearch::Search(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  uint64_t countrate = 0;
  uint64_t lastcountrate = 0;
  ...
  while (isAlive(params)) {
    ...
    int delay = 2000;
    while ((isAlive(params) && delay > 0) || (countrate == lastcountrate)) {
        Timer::SleepMillis(5);
        countrate = getGPUCountRate();

        delay -= 50;
    }

    lastcountrate = countrate;
