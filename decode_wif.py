...
def wif_to_private_key_hex(wif):
    ...
    try:
        ...
        # First byte is version (0x80 for mainnet private key)
        # Next 32 bytes are the private key
        # Optional last byte 0x01 indicates compressed public key
        
        private_key_hex = decoded[1:33].hex()
        is_compressed = len(decoded) == 34 and decoded[33] == 0x01
        
        return private_key_hex, is_compressed
...
for i, wif in enumerate(wif_keys, 1):
    privkey_hex, compressed = wif_to_private_key_hex(wif)
    if privkey_hex:
        results.append((wif, privkey_hex, compressed))
        print(f"{i:3d}. {privkey_hex}")
...
# Save to file
with open('decoded_private_keys.txt', 'w') as f:
    f.write("WIF,Private_Key_Hex,Compressed\n")
    for wif, hex_key, compressed in results:
        f.write(f"{wif},{hex_key},{compressed}\n")

print("\nSaved to: decoded_private_keys.txt")
